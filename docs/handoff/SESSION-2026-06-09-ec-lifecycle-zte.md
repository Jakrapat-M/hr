...
3. **MCP Playwright was locked** ("Browser is already in use") by parallel agents all session → used **standalone Playwright** scripts: `import pkg from '<repo>/node_modules/playwright/index.js'; const {chromium}=pkg;` (CJS default import), seed `humi-auth` + `page.route('**/api/auth/session', …'{}')`.
