...
> MK V — Researcher report สำหรับ Cnext HR (Central Group HR platform replacement).
...
Design file ชื่อ "Shelfly" เป็น placeholder ที่ Claude Design ตั้งในรอบแรก (เพราะ "Cnext" เป็นบริษัท HR จริงในแคนาดา). หลังจากนั้น user instruct ให้ rename กลับเป็น **"Cnext"** — ทุก screen + login + sidebar ใช้ wordmark `Cnext` แล้ว. รอบ final ของ bundle นี้คือ Cnext branding ไม่ใช่ Shelfly. ชื่อไฟล์ยังค้าง `Shelfly HR.html` ไว้เฉยๆ.

> **Implication for Cnext HR project**: design นี้ถูก tailor ให้เป็น "Cnext" อยู่แล้ว (wordmark + sidebar brand mark "Hum" + gumdrop `i`). Asset reuse ได้ตรงโดยไม่ต้อง rebrand.
...
### 🔴 NO-RED audit (Cnext constraint check)
...
| `clay` accent | `#C8553D` | ⚠️ เหมือน `--danger` — เป็น accent option → **ต้องตัดทิ้งถ้าใช้กับ Cnext** (Rungrote = NO RED) |
...
**Recommendation สำหรับ Cnext**:
- ลบ `clay` ออกจาก accent options
- Map `--danger` ไป `#C99A2E` (mustard) หรือ `#8A4A2F` (burnt brown) แทน
- ยืน `--coral` ไว้ได้ (peach/salmon) แต่ต้อง double-check กับ Rungrote — ถ้าไม่โอเค swap เป็น `#D8A15C` (caramel) หรือใช้ Cnext palette ตรงจาก logo decks (teal/emerald/indigo/plum/amber ที่ recolor script ทำไว้แล้ว)

## Typography

### Font stack (ปัจจุบัน Cnext branding ใน bundle)
...
> ✅ Bonus: CPN = Central Pattana font! ทำให้ Cnext ไม่ต้อง license font ใหม่ — **พร้อมใช้ได้ทันที** กับ Central Group branding guideline.
...
│   ├── .brand (wordmark "Cnext" + gumdrop mark)
...
| Brand wordmark | `.brand .wordmark` | Logo lockup | "Cnext" + `ShelflyMark` (gumdrop person SVG) replacing the `i` dot; teal accent on mark |
...
**Empty states / loading states**: ⚠️ ไม่พบ explicit pattern ใน source. มีแค่ filter/tabs ที่ render empty list เงียบๆ. **Gap สำหรับ Cnext**: ต้องออกแบบเพิ่ม (skeleton loader + empty illustration).
...
- **Tweaks floating panel** (`.tweaks`) — accent swatch selector → ปรับ theme live. ถ้าทำ Cnext ให้ทีม Branding ทดลอง 36+ recolor variants ได้ live ใน prototype, เอาไปเสนอ Rungrote ได้เลย
...
- ทุก email domain `@cnext.shop` → change to Central Group equivalent (`@centralgroup.com` / `@cpn.co.th`)
...
- Brand placeholder text "Cnext" — ถ้า final brand name ยังไม่ lock (ดู `wiki/concepts/cnext-branding.md` — STATUS pending Rungrote feedback) ให้ใช้ placeholder component ที่ swap ง่าย
...
- **Store manager persona**: ต้นฉบับ narrate ผ่าน "อวา เรเยส · ผู้จัดการร้านสาขาฮัมเบอร์" — Cnext HR ต้อง support **multi-persona** (corporate HR, mall GM, store manager, hourly staff, executive) → role switcher ต้องมี
- **"Cnext" references ใน URL/email** — change domain; ทุก screen ใช้ `@cnext.shop`
...
**Action item**: ถ้า Cnext HR ตัดสินใจใช้ bundle นี้เป็น baseline, แนะนำ copy เข้า `hr/docs/design-ref/shelfly-bundle/` (สำหรับ reference, ไม่ใช่ import ตรง) + แปลง `styles.css` tokens เป็น Tailwind config หรือ shadcn CSS variables.

## Summary for MK XLII PO

- **Fetch**: ✅ Success ผ่าน curl + gunzip + tar; bundle มี 10 screens + design tokens + CPN fonts + chat journey
- **Brand name in bundle**: **Cnext** (final state — "Shelfly" แค่ placeholder รอบแรก)
...
- **Top risks**: retail-centric copy, `@cnext.shop` email domain, Babel-in-browser (not prod-ready), `<div onClick>` a11y gap, hardcoded Thai (no i18n)
