# Cnext Primitives — Component Reference

> Shared primitives ใน `src/frontend/src/components/cnext/` สำหรับ Cnext redesign
...
```tsx
import { Button, Card, CardTitle, Nav, DataTable, FormField, FormInput } from '@/components/cnext';
```
...
<Card tone="accent" size="lg">
  <CardTitle>เริ่มต้นใช้งาน Cnext</CardTitle>
</Card>
```
...
<Nav
  ariaLabel="เมนูหลัก"
  brand={<span className="font-display text-display-h3">Cnext</span>}
  sections={[
    {
      id: 'main',
      label: 'หลัก',
...
ใน components ใดๆ — ดู guardrail list ใน `specs/cnext-frontend-redesign.md` + grep check ใน
