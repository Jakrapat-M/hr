# Plan: Cnext Frontend Redesign — Sprint Adapt from Shelfly Bundle

## Task Description

Redesign Cnext HR (Central Group HR platform replacement) frontend โดย **adapt** design quality + interaction patterns จาก Shelfly/Cnext design handoff bundle (10 screens + design tokens + CPN fonts) ที่ persist อยู่ใน `hr/docs/design-ref/shelfly-bundle/`. Scope คือ **visual re-skin + core screens 5 หน้า** ของ existing `hrms-frontend` (Next.js 16 + Tailwind v4) — ไม่แตะ backend schema, API routes, prisma migrations, MSAL auth (parallel track กับ Sprint 1 backend).

**ไม่ใช่** full clone ของ bundle — คง Cnext identity (Central Group HR multi-persona) แทนที่ retail-centric copy/assets. คำว่า "Shelfly" ใน bundle เป็น placeholder รอบแรก — final state = Cnext แล้ว แต่ branding/color audit ต้องทำใหม่ตาม Rungrote NO-RED constraint.
...
- ≥ 5 core Cnext screens implemented (Dashboard/หน้าหลัก, Employee List/บุคลากร, Employee Detail, Org Chart/ผังองค์กร, Settings) — visually adapt จาก bundle
...
src/frontend/src/
├── app/
│   ├── globals.css              ← Tailwind v4 @theme tokens (Cnext palette) — REPLACES existing Precision Cool
...
├── components/
│   └── cnext/                    ← NEW directory — Cnext redesign primitives
...
└── lib/
    └── cnext-theme.ts            ← Accent color helper (teal default + indigo alt only — NO clay)
```

### Key Design Decisions

**D1. Tailwind v4 `@theme` SSOT — not tailwind.config.ts**
Existing `globals.css` ใช้ v4 pattern (`@theme { --color-* }`). Redesign จะ **replace** Precision Cool palette ทั้ง block ด้วย Cnext tokens — ไม่สร้าง `tailwind.config.ts` (v4 ไม่ต้อง). rationale: ทีมเดิมใช้ v4 pattern แล้ว ให้ consistent.

**D2. NO-RED palette scrub — hard requirement**
Map bundle tokens → Cnext palette:

| Bundle token | Hex | Cnext decision |
|---|---|---|
...
existing `--color-brand: #C8102E` (Central retail red) จะถูกเปลี่ยนเป็น Cnext teal `#1FA8A0`. Sprint 1 backend + auth ไม่ได้ผูกกับ color var นี้ จึงปลอดภัยที่จะ replace.
...
Sprint 1 backend (Org CRUD + ltree + 13 org types) เดิน parallel อยู่. Redesign ใช้ mock/fixture data ใน `src/frontend/src/lib/cnext-mock.ts` — ไม่ fetch จริง.

**D9. i18n-ready structure (not full extraction)**
เพิ่ม copy ใหม่เข้า `messages/th.json` + `messages/en.json` (existing next-intl setup). Hardcoded Thai ใน bundle → extract เป็น message keys ที่ structured (`cnext.dashboard.greeting`, `cnext.nav.people`, etc.). **ไม่ต้อง** extract ทั้ง codebase รอบนี้.

**D10. Logo asset pick (not generate)**
เลือก Cnext logo variant จาก `stark/projects/hr-platform-replacement/logo-concepts/v2/` — pick 1 cool (teal/indigo) + 1 warm (amber/gold) เป็น fallback. Copy เข้า `src/frontend/public/brand/cnext-logo.svg` + `cnext-wordmark.svg`. ห้าม generate logo ใหม่.
...
- `src/frontend/src/app/globals.css` — **REPLACE** Precision Cool `@theme` block ด้วย Cnext tokens
...
- `src/frontend/messages/th.json`, `messages/en.json` — add Cnext copy keys
...
- `src/frontend/public/brand/cnext-logo.svg`, `cnext-wordmark.svg` — picked from logo-concepts v2
- `src/frontend/src/components/cnext/Button.tsx`
- `src/frontend/src/components/cnext/Card.tsx`
- `src/frontend/src/components/cnext/Sidebar.tsx`
- `src/frontend/src/components/cnext/Topbar.tsx`
- `src/frontend/src/components/cnext/DataTable.tsx`
- `src/frontend/src/components/cnext/FormField.tsx`
- `src/frontend/src/components/cnext/KpiCard.tsx`
- `src/frontend/src/components/cnext/Eyebrow.tsx`
- `src/frontend/src/components/cnext/Blob.tsx`
- `src/frontend/src/components/cnext/Avatar.tsx`
...
- `src/frontend/src/lib/cnext-mock.ts` — mock fixtures (employees, org, approvals, announcements)
- `src/frontend/src/lib/cnext-theme.ts` — theme util (accent teal/indigo only)
- `src/frontend/src/__tests__/cnext/Button.test.tsx` — component tests
- `src/frontend/src/__tests__/cnext/Card.test.tsx`
- `src/frontend/src/__tests__/cnext/Sidebar.test.tsx`
- `src/frontend/src/__tests__/cnext/DataTable.test.tsx`
- `src/frontend/src/__tests__/cnext/FormField.test.tsx`
- `src/frontend/e2e/cnext-screens.spec.ts` — Playwright screenshot capture (5 screens)
- `src/frontend/e2e/cnext-a11y.spec.ts` — axe-core accessibility scan
...
- `stark/projects/hr-platform-replacement/logo-concepts/v2/` — Cnext logo variants
...
- **Component Library Builder**
  ...
  - Role: 10 primitives ใน `components/cnext/` (Button/Card/Sidebar/Topbar/DataTable/FormField/KpiCard/Eyebrow/Blob/Avatar) — typed, a11y-clean, ไม่มี `<div onClick>`
...
- **Screen Builder C (Settings + i18n copy)**
  ...
  - Agent: frontend-design skill
  - Role: `settings/page.tsx` + extend `messages/th.json` + `messages/en.json` with Cnext copy keys
...
- **Files**:
  ...
  - `src/frontend/public/brand/cnext-logo.svg`, `cnext-wordmark.svg` (pick from logo-concepts/v2)
  - `src/frontend/src/lib/cnext-theme.ts` (accent helper)
- **Actions**:
  - Replace Precision Cool `@theme` block with Cnext tokens (teal `#1FA8A0` primary, indigo `#5B6CE0` alt, cream `#F6F1E8` bg, navy ink `#0E1B2C`)
...
- **Parallel**: false
- **Files**: 10 files in `src/frontend/src/components/cnext/`
- **Actions**:
...
- **Files**:
  ...
  - `src/frontend/src/lib/cnext-mock.ts` (employees + approvals + announcements fixtures)
- **Actions**:
  ...
  - All widgets use cnext/* primitives — no custom one-offs
...
- **Actions**:
  ...
  - Extract all new Thai strings to `messages/th.json` under `cnext.*` namespace
  - Mirror structure in `messages/en.json` (English fallback strings)
  - Update 3 other screen builders to use `useTranslations('cnext')` hook (light touch — not full repo extract)
...
- **Parallel**: false
- **Files**:
  - `src/frontend/src/__tests__/cnext/Button.test.tsx`
  - `src/frontend/src/__tests__/cnext/Card.test.tsx`
  - `src/frontend/src/__tests__/cnext/Sidebar.test.tsx`
  - `src/frontend/src/__tests__/cnext/DataTable.test.tsx`
  - `src/frontend/src/__tests__/cnext/FormField.test.tsx`
  - `src/frontend/e2e/cnext-screens.spec.ts`
  - `src/frontend/e2e/cnext-a11y.spec.ts`
- **Actions**:
...
- **Actions**:
  ...
  - `cd src/frontend && npx playwright test e2e/cnext-screens.spec.ts` — capture 5 screenshots
  - `cd src/frontend && npx playwright test e2e/cnext-a11y.spec.ts` — axe-core report
  ...
  - Write AC table to `docs/cnext-redesign-validation.md` with pass/fail per AC + evidence
...
- **AC-2**: ≥ 5 core Cnext screens implemented (`home`, `people` list, `people/[id]` detail, `org-chart`, `settings`) — ทุกหน้าเปิดได้บน `npm run dev`, ไม่มี console error, render ครบตาม bundle reference (ring KPI, border-left KPI cards, split layout for org chart, sticky topbar). Verifiable: Playwright captures 5 screenshots at 1920x1080 with no error overlay.
...
- **AC-5**: **0 retail-centric copy** — `rg -n -i 'store|cashier|POS|inventory|\bshift\b.*wage' src/frontend/src/app/\[locale\]/` returns 0 matches in new Cnext screens. Mock personas include corporate HR + mall ops + warehouse roles (not retail-only).
- **AC-6**: **Thai-primary** — ทุก visible page title + nav label + primary copy ใน 5 หน้าเป็นภาษาไทย (English เฉพาะ tech acronyms เช่น "HR", "SSO", "API"). Verifiable: MK IV visual review + `messages/th.json` contains all `cnext.*` keys used in screens.
- **AC-7**: **A11y baseline** — `rg -n '<div[^>]*onClick' src/frontend/src/components/cnext/ src/frontend/src/app/\[locale\]/` returns 0 matches. axe-core Playwright scan reports 0 serious/critical violations. Lighthouse a11y score ≥ 90 on all 5 screens. No global `*` CSS reset (Rule 26b): `rg '^\s*\*\s*\{' src/frontend/src/app/globals.css` returns 0 matches.
- **AC-8**: Component library extracted — 10 primitives in `src/frontend/src/components/cnext/` (Button, Card, Sidebar, Topbar, DataTable, FormField, KpiCard, Eyebrow, Blob, Avatar) — all typed, all use class-variance-authority variants, reused ≥ 2x across 5 screens each (verified by Code Reviewer).
...
- `cd src/frontend && npx playwright test e2e/cnext-screens.spec.ts` — 5-screen screenshot capture
- `cd src/frontend && npx playwright test e2e/cnext-a11y.spec.ts` — axe-core scan
...
- `rg -n '<div[^>]*onClick' src/frontend/src/components/cnext/ src/frontend/src/app/\[locale\]/home src/frontend/src/app/\[locale\]/people src/frontend/src/app/\[locale\]/org-chart src/frontend/src/app/\[locale\]/settings` — expect 0 matches (AC-7)
...
- Full i18n extraction of existing (non-Cnext) screens — extend th.json/en.json for new screens only
...
- Dark mode — existing code has `@variant dark` hook but Cnext dark palette not yet specified
...
- Email/domain rebrand (`@cnext.shop` placeholders) — replace with neutral `@example.com` in mock only; real domain decision = ops concern
