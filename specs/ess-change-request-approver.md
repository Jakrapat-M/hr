...
`/admin/self-service/` has config subsurfaces (field-config / visibility / mandatory / readonly / quick-actions / tiles / layout) but **no approver queue**. `cnext-profile-slice.ts` has `adminApprove(id)` and `adminReject(id)` actions that are **currently called from zero UI consumers** — they exist but no one uses them.
...
**DO NOT unify** with `/workflows/page.tsx` (generic leave/OT via `use-workflows` hook) or `/ess/workflows/page.tsx` (5-persona journey via `workflow-approvals` store). Those are separate domains. `cnext-profile-slice.pendingChanges` is the ONE source for profile-data CRs.

## Objective

Ship a greenfield approver surface that closes the employee-to-admin loop for personal-data CRs introduced by #53, adding the missing approve/reject actions with audit reason. Must route through `cnext-profile-slice`, must group by `sectionKey`, must preserve C7 SSoT boundary vs the two coexisting workflow systems.

## Tech Stack

- Next.js 16 App Router (`src/frontend/src/app/[locale]/admin/change-requests/`)
- React 19 + TypeScript 5
- Zustand 4 (existing `cnext-profile-slice`, extend additively)
- `@/components/cnext` primitives — Button, Card, Modal, FormField
...
- `src/frontend/src/stores/cnext-profile-slice.ts` — existing pendingChanges state, adminApprove/adminReject actions (lines 125, 137, 212, 216); FileAttachment shape (line ~25)
...
- `src/frontend/src/stores/cnext-profile-slice.ts` — add `adminApproveWithReason(id, reason?)` + `adminRejectWithReason(id, reason?)`; keep existing `adminApprove`/`adminReject` as thin wrappers; extend `PendingChange` with optional `reason?: string`. NO version bump required (additive-only; v2 shape includes optional field).
...
- Tests: `src/frontend/src/__tests__/approver-list.test.tsx`, `reason-modal.test.tsx`, `cnext-profile-slice.approve-reject.test.ts`
...
|---|---|---|
| **MK III — Builder** | T1 | Extend cnext-profile-slice (PendingChange.reason + adminApproveWithReason/RejectWithReason additive variants; preserve existing wrappers) |
...
|---|---|---|---|---|
| T1 | Extend `cnext-profile-slice.ts`: add `reason?: string` to PendingChange; add `adminApproveWithReason(id, reason?)` + `adminRejectWithReason(id, reason?)` that set reason+approvedAt+status; change existing `adminApprove(id)`/`adminReject(id)` to delegate to With-Reason variants with undefined reason (preserve callers). Max 1 file. | — | MK III — Builder | false |
...
- **AC-3** — cnext-profile-slice extended additively (invariant)
...
- Store unification (cnext-profile-slice vs use-workflows vs workflow-approvals) — separate task
