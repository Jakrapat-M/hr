...
**Carry-over note**: `/requests` "approvals" tab uses static `CNEXT_APPROVAL_QUEUE` mock and is *not* wired to either of the real approval stores. This is technical debt but not a navigation duplication — left for a future cleanup pass.
...
- **Consumer**: `src/frontend/src/components/cnext/shell/PersonaSwitcher.tsx:32, 78, 114` (renders `badge.label` as the chip)

### Current chip-label generation (lines 104-115)

```ts
export const PERSONA_BADGE: Record<string, { label: string; tone: string }> = {
  'admin@cnext.test':    { label: 'Admin',    tone: 'cnext-tag--ink' },     // ← (a)
  ...
  // T7 — SF-canonical
  'ken@cnext.test':      { label: 'HR Admin', tone: 'cnext-tag--ink' },     // ← (b) MISMATCH
...
- Line 105: `'admin@cnext.test'` → label `"Admin"` for `roles: ['hr_admin', 'hr_manager', 'spd', 'hrbp', 'manager', 'employee']` (super-user)
- Line 111: `'ken@cnext.test'` → label `"HR Admin"` for `roles: ['hr_admin', 'employee']`

Both effectively render as the **HR Admin** role chip family (same `tone: 'cnext-tag--ink'`), but with different visible text. User reports this as inconsistent in the demo switcher dropdown.
...
2. **`"Admin"` for both** — shorter, fits in the chip pill better; matches the existing topbar tone class `cnext-tag--ink` semantically.
...
- `admin@cnext.test` is a *super-user* with 6 roles — labeling it just `"Admin"` *under-states* the privilege (it's actually the HR Admin landing path that fires per `ROLE_LANDING[0]`).
...
```diff
-  'admin@cnext.test':    { label: 'Admin',    tone: 'cnext-tag--ink' },
...
- Run `rg -n "Admin" src/frontend/src/components/cnext/shell/__tests__/` and `rg -n "PERSONA_BADGE" src/frontend/src/__tests__/` to scan. If found, update assertion strings to `"HR Admin"`. (Not load-bearing — the badge is purely visual.)
...
- **Files deleted**: 0
- **Routes redirected**: 0
- **Persona-label fixes**: 1 (admin@cnext.test chip: `Admin` → `HR Admin`)
