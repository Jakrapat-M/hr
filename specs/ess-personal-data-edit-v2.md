# ESS Self-Service Personal Data Edit — v2 (extend cnext-profile-slice)

**Tracking**: aeiouboy/hr#53
**Seed**: `specs/ess-personal-data-edit-v2.seed.json`
**Supersedes**: `specs/ess-personal-data-edit.md` (v1 — halted per C7 SSoT: duplicated cnext-profile-slice that already exists from ironteam #12)
...
- `/profile/me` = หน้า canonical 1385 บรรทัดจาก ironteam #12 ที่ **edit + CR submit ทำงานอยู่แล้ว** ผ่าน `useCnextProfileStore` (Zustand persist, name `cnext-profile-v1`)
...
งานนี้ = **extend existing cnext-profile-slice + existing /profile/me/page.tsx**. ห้ามสร้าง parallel store, parallel modal, parallel pending-change type. **C7 SSoT สำคัญกว่าทุกอย่างใน spec นี้**.

Approver UI (หน้า HR อนุมัติ CR) = out of scope, ไปที่ #54.

## Objective

ส่งมอบ 3 new sections + 2 refactors บน `/profile/me` ที่ extend cnext-profile-slice ผ่าน `version: 2` + `migrate()`, ผ่าน G3 gate 10/10 ACs, vitest green, โดย:

- ไม่สร้าง store / modal / type ใหม่ที่ซ้ำกับ `cnext-profile-slice` / `PendingChange` / Modal in page
...
- **State**: Zustand 4 + persist (existing `cnext-profile-slice.ts` — bump `version: 2` + `migrate()`)
- **UI primitives**: `@/components/cnext` (Button/Card/Modal/FormField)
- **Attachments**: `@/components/cnext/FileUploadField` (existing, in-use) OR `@/components/admin/AttachmentDropzone` (newer Ken directive 2026-04-24). Builder picks one per section; spec v2 allows either but demands ONE consistent choice per section + rationale comment.
- **i18n**: next-intl via `messages/th.json` + `messages/en.json` (append new keys only; don't touch existing `cnextProfile.*`, `profileEdit.*`, `pending.*` keys)
- **Tests**: vitest + @testing-library/react + jsdom

## Relevant Files

### Read-only (understand shape before editing)
- `src/frontend/src/stores/cnext-profile-slice.ts` — existing store (line 7-13 ProfileDraft flat shape; line 137-153 submitChangeRequest; line 189-200 persist config name `cnext-profile-v1`)
- `src/frontend/src/app/[locale]/profile/me/page.tsx` — existing 1385-line profile page (uses `useCnextProfileStore`, `FileUploadField`, 5-tab switcher with SLICE_TO_PANEL map)
- `src/frontend/src/app/[locale]/employees/me/page.tsx` — 15-line redirect (do not touch)
- `src/frontend/src/components/cnext/FileUploadField.tsx` — existing attachment primitive
...
- `src/frontend/src/stores/cnext-profile-slice.ts` — extend `ProfileDraft` shape, bump `version: 2`, add `migrate()`, extend `PendingChange` with optional `sectionKey?: SectionKey`. ~60 lines added, ~4 lines touched. Max 1 file.
...
- `cnext-profile-v1` consumer components OTHER than /profile/me/page.tsx (don't refactor globally)
- Approver UI — #54
...
|---|---|---|
| **MK III — Builder** | T1 | Extend cnext-profile-slice (shape + migrate + sectionKey); edit /profile/me to render 3 new section blocks + upgrade Contact Info |
...
| T5 | Code Review (C1 surgical — verify no parallel store/modal/type; C3 — no new abstractions over existing cnext-profile-slice; C7 — single source; C10 — Thai labels). Run `bun run test` via JARVIS Bash. Capture `test-artifacts/ironteam-53-v2/test-output.txt`. Start dev server + browser-harness screenshots for AC-1..AC-7 on `/profile/me`. Emit `outcomes.json` for G3 gate. Validate Final verdict. | T4 | MK IV — Validator | false |
...
- **AC-10** — Vitest suite green on new tests + no regression on existing cnext tests (compare pass count before/after)
...
- Real backend persistence / Prisma migration — existing FE mock via cnext-profile-slice
- Push/email notifications — #54 or later
- Bulk edit / HR admin override — not ESS
- Global refactor of other consumers of cnext-profile-slice — surgical to /profile/me only
