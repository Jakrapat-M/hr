...
describe('useProfileEdit — initial state', () => {
  it('draft ต้องว่างทุก field, isDirty=false, isSubmitting=false', () => {
    ...
    // address was refactored into 8 sub-fields (Thai address standard)
    expect(result.current.draft.addressHouseNo).toBe('')
    expect(result.current.draft.addressSubdistrict).toBe('')
    expect(result.current.draft.addressProvince).toBe('')
