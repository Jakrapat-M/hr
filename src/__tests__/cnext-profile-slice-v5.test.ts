/**
 * T3 (#90) — cnext-profile-slice v4 → v5 migrate
 *
...
describe('T3 #90 — cnext-profile-slice v4→v5 migration', () => {
  ...
  test('migrate v4 → v5 preserves v4 dependents schema verbatim', async () => {
    ...
    const migrate = useCnextProfileStore.persist.getOptions().migrate;
  ...
  test('field-level change request (first_name_th) flows through PendingChange.field', async () => {
    const { useCnextProfileStore } = await import('@/stores/cnext-profile-slice');
    const store = useCnextProfileStore;
