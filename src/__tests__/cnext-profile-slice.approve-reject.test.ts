/**
 * cnext-profile-slice.approve-reject.test.ts
...
function seedChange(overrides: { field?: string; sectionKey?: 'personal' | 'contact' | 'address' | 'emergencyContact' | 'bank' } = {}): string {
  const { submitChangeRequest } = useCnextProfileStore.getState();
...
describe('adminApproveWithReason', () => {
  it('Test 1: sets status=approved + approvedAt non-null + reason stored', () => {
    ...
    useCnextProfileStore.getState().adminApproveWithReason(id, 'ตรวจสอบแล้ว');
  ...
  it('Test 2: no reason arg → status=approved + reason=undefined', () => {
    ...
    useCnextProfileStore.getState().adminApproveWithReason(id);
...
describe('adminRejectWithReason', () => {
  it('Test 3: sets status=rejected + reason stored', () => {
    ...
    useCnextProfileStore.getState().adminRejectWithReason(id, 'ข้อมูลไม่ครบ');
...
describe('nonexistent id guard', () => {
  it('Test 6: adminApproveWithReason on nonexistent id → console.warn + no state mutation', () => {
    ...
    useCnextProfileStore.getState().adminApproveWithReason('nonexistent-id-xyz', 'x');
...
describe('independent approvals — immutability of untouched items', () => {
  it('Test 7: multiple ids each flip independently; untouched items stay immutable', () => {
    ...
    const beforeState = useCnextProfileStore.getState().pendingChanges;

    useCnextProfileStore.getState().adminApproveWithReason(id1, 'ผ่าน');
    useCnextProfileStore.getState().adminRejectWithReason(id2, 'ไม่ผ่าน');
...
describe('withdrawPendingChange', () => {
  it('allows employee withdrawal only while pending and keeps history row', () => {
    ...
    useCnextProfileStore.getState().withdrawPendingChange(id);
