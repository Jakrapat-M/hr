/**
 * cnext-mock-data-years-in-x.test.ts — Years-in-X integration tests (hr#82 BRD #168)
 * Framework: Vitest + jsdom
 *
 * Verifies that CNEXT_MY_PROFILE exposes:
...
describe('hr#82 BRD #168 — Years-in-X integration', () => {
  ...
  it('AC-1 — exposes lifecycleEvents array on CNEXT_MY_PROFILE', () => {
    expect(CNEXT_MY_PROFILE.lifecycleEvents).toBeDefined()
    expect(Array.isArray(CNEXT_MY_PROFILE.lifecycleEvents)).toBe(true)
    expect(CNEXT_MY_PROFILE.lifecycleEvents.length).toBeGreaterThanOrEqual(4)

    // I11 invariant: HIRE event must exist
    const hire = CNEXT_MY_PROFILE.lifecycleEvents.find((e) => e.type === 'HIRE')
  ...
  it('AC-1 (detail) — seed contains HIRE, CHANGE_POSITION, CHANGE_JOB, PROMOTION types', () => {
    const types = CNEXT_MY_PROFILE.lifecycleEvents.map((e) => e.type)
  ...
  it('AC-2 — job array contains all 5 Years-in-X labels', () => {
    const labels = CNEXT_MY_PROFILE.job.map(([label]) => label)
  ...
  it('AC-3 — Years-in-X values follow buildDisplay format (N ปี M เดือน / N ปี / M เดือน / 0 เดือน)', () => {
    const yearsValues = CNEXT_MY_PROFILE.job
      .filter(([label]) =>
  ...
  it('AC-4 — no hardcoded "X.X ปี (computed)" strings remain in job array', () => {
    const allValues = CNEXT_MY_PROFILE.job.map(([, value]) => value).join('|')
