 ...
 *
 * Real fixtures from cnext-mock-data used (rule 63).
 */
...
// Same pattern as layout-integration.test.tsx + cnext-responsive.test.tsx.
...
describe('AC-3 — /profile/me functional', () => {
  ...
  it('clicking employment tab changes activeTab in Zustand store', async () => {
    ...
    const { useCnextProfileStore } = await import('@/stores/cnext-profile-slice');
    // Initial tab is personal
    expect(useCnextProfileStore.getState().activeTab).toBe('personal');

    // Click employment tab (label from t('tabJob') = 'การจ้างงาน')
    const jobTab = screen.getByText('การจ้างงาน');
    await user.click(jobTab);

    expect(useCnextProfileStore.getState().activeTab).toBe('employment');
  });
  ...
  it('clicking edit button enters draft mode', async () => {
    ...
    const { useCnextProfileStore } = await import('@/stores/cnext-profile-slice');
    expect(useCnextProfileStore.getState().isEditing).toBe(false);

    const editBtn = screen.getByText('แก้ไข');
    await user.click(editBtn);

    expect(useCnextProfileStore.getState().isEditing).toBe(true);
  });
  ...
  it('updateDraft updates draft.nickname in store', async () => {
    ...
    act(() => {
      ...
      useCnextProfileStore.getState().updateDraft({ nickname: 'ทดสอบ' });
    });
    expect(useCnextProfileStore.getState().draft.nickname).toBe('ทดสอบ');
  });
});
...
describe('AC-6 — /announcements functional', () => {
  ...
  it('clicking pin button toggles pinned state in store', async () => {
    ...
    const { useCnextAnnouncementsStore } = await import('@/stores/cnext-announcements-slice');
    const initialPinnedCount = useCnextAnnouncementsStore.getState().pinned.length;
    ...
    const afterPinnedCount = useCnextAnnouncementsStore.getState().pinned.length;
  ...
  it('setFilter changes activeFilter in store', async () => {
    const { useCnextAnnouncementsStore } = await import('@/stores/cnext-announcements-slice');
    act(() => {
      useCnextAnnouncementsStore.getState().setFilter('policy');
    });
    expect(useCnextAnnouncementsStore.getState().activeFilter).toBe('policy');
  });
});
...
describe('AC-13 — /timeoff functional', () => {
  ...
  it('timeoff store submit appends new item to history', async () => {
    const { useTimeoffStore } = await import('@/stores/cnext-timeoff-slice');
...
describe('AC-10 — /benefits-hub functional', () => {
  ...
  it('toggleEnroll removes id from enrolled set when already enrolled', async () => {
    const { useBenefitsStore } = await import('@/stores/cnext-benefits-slice');
...
describe('AC-3 — /requests functional', () => {
  ...
  it('submit action appends to submissions in store', async () => {
    const { useRequestsStore } = await import('@/stores/cnext-requests-slice');
...
describe('AC-5 — /goals functional', () => {
  ...
  it('create adds goal item to store', async () => {
    const { useGoalsStore } = await import('@/stores/cnext-goals-slice');
...
describe('AC-6 — /learning-directory functional', () => {
  ...
  it('setQuery updates query in learning store', async () => {
    const { useLearningStore } = await import('@/stores/cnext-learning-slice');
    act(() => {
...
describe('AC-10 — /org-chart functional', () => {
  ...
  it('setQuery updates query in orgchart store', async () => {
    const { useOrgChartStore } = await import('@/stores/cnext-orgchart-slice');
    act(() => {
...
describe('AC-13 — /integrations functional', () => {
  ...
  it('toggle flips enabled state for integration id', async () => {
    const { useIntegrationsStore } = await import('@/stores/cnext-integrations-slice');
...
describe('AC-13 — /login functional', () => {
  ...
  it('submitting login form calls router.push to /th/home', async () => {
    ...
    // Login form uses cnext-field label wrapping (no htmlFor/id) — query by input type directly.
    // default email is admin@cnext.test (hr_admin → /admin); use employee@cnext.test (→ /home).
    const emailInput = container.querySelector('input[type="email"]') as HTMLInputElement;
    await user.clear(emailInput);
    await user.type(emailInput, 'employee@cnext.test');

    const pwInput = container.querySelector('input[type="password"]') as HTMLInputElement;
    await user.type(pwInput, 'employee2026');

    // Login page uses useTranslations('cnextLogin') — the test mock returns key
...
//
// Bug: `.cnext-app { grid-template-columns: 1fr }` lived outside any @layer,
// while the desktop override `@media (min-width: 1024px) { .cnext-app { ...
...
// `.cnext-app` must NOT be nested inside @layer components.
// ─────────────────────────────────────────────────────────────────────────────
describe('REGRESSION — globals.css desktop @media must escape @layer', () => {
  it('@media (min-width: 1024px) for .cnext-app is at top level (not inside @layer)', async () => {
    ...
    // none of them contain the desktop override for .cnext-app.
    ...
    while ((m = layerRe.exec(css)) !== null) {
      ...
      // The block must NOT contain the lg+ media query targeting .cnext-app.
      // Use [\s\S] instead of /s flag (target compatibility — TS1501).
      const hasBuggyNesting =
        /@media\s*\([^)]*min-width:\s*1024px[^)]*\)\s*\{[\s\S]*?\.cnext-app/.test(block);
      expect(hasBuggyNesting).toBe(false);
    }

    // And the override must exist somewhere at top level
    const topLevel = /@media\s*\([^)]*min-width:\s*1024px[^)]*\)\s*\{[\s\S]*?\.cnext-app[\s\S]*?grid-template-columns/;
...
describe('REGRESSION — Hamburger + Mobile Drawer a11y', () => {
  ...
  it('Hamburger button has aria-expanded + aria-controls + dynamic aria-label', async () => {
    ...
    const hamburger = document.querySelector(
      'button[aria-controls="cnext-mobile-drawer"]',
  ...
  it('Mobile drawer renders with role="dialog" + aria-modal + matching id', async () => {
    ...
    expect(document.getElementById('cnext-mobile-drawer')).toBeNull();
    ...
    await waitFor(() => {
      const drawer = document.getElementById('cnext-mobile-drawer');
...
// (Bug 2026-04-22 "double cnext"): without a match, resolveTitle() falls back
// to 'Cnext' which duplicates the sidebar logo and reads as a visual bug.
...
describe('REGRESSION — Sidebar NAV ↔ AppShell TITLE_MAP parity', () => {
  it('every internal sidebar href has a matching TITLE_MAP prefix', async () => {
    ...
    const sidebarSrc = await fs.readFile(
      path.resolve(__dirname, '../components/cnext/shell/Sidebar.tsx'),
      'utf-8',
    );
    const appShellSrc = await fs.readFile(
      path.resolve(__dirname, '../components/cnext/shell/AppShell.tsx'),
      'utf-8',
    );
