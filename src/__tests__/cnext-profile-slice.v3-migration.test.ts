/**
 * cnext-profile-slice.v3-migration.test.ts
...
function seedV2(overrides?: Record<string, unknown>) {
  ...
  localStorageMock.setItem('cnext-profile-v1', JSON.stringify(v2Blob));
}
...
describe('AC-6: store version ใน localStorage ต้องเป็น 4', () => {
  it('AC-6: หลัง setState + persistMiddleware write → localStorage blob ต้องมี version>=3', () => {
    ...
    if (raw) {
      ...
      // hr#85 bumped v3→v4 for 'dependents' SectionKey + extended CnextDependent shape
      ...
      const storeSource = require('fs').readFileSync(
        require('path').resolve(__dirname, '../stores/cnext-profile-slice.ts'),
        'utf-8',
      );
