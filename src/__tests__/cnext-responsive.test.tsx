 ...
 * AC-2  nav-active   — Cnext Nav mounted (no regression from #3)
...
import { AppShell } from '@/components/cnext/shell/AppShell';
...
describe('AC-4 + AC-8 — Topbar no-regression + compressed tokens', () => {
  ...
  it('Topbar renders h2 page title', async () => {
    const { Topbar } = await import('@/components/cnext/shell/Topbar');
...
describe('AC-5 + AC-9 — CommandPalette fullscreen/modal tokens', () => {
  ...
  it('CommandPalette backdrop has fixed inset-0 (full-screen mobile base)', async () => {
    const { CommandPalette } = await import('@/components/cnext/shell/CommandPalette');
    const { container } = render(
...
describe('AC-6 — touch-target: icon buttons in Topbar', () => {
  ...
  it('Topbar icon buttons carry cnext-icon-btn class (44px tap region via CSS)', async () => {
    const { Topbar } = await import('@/components/cnext/shell/Topbar');
    const { container } = render(<Topbar title="หน้าหลัก" />);
    // cnext-icon-btn CSS class provides min 44px touch target
    const iconBtns = container.querySelectorAll('.cnext-icon-btn');
