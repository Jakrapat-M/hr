...
import type { CnextClaimHistoryItem } from '@/lib/cnext-mock-data';

const approvedRow: CnextClaimHistoryItem = {
  id: 'cl-1',
  date: '15 เม.ย. 2569',
...
const pendingRow: CnextClaimHistoryItem = { ...approvedRow, id: 'cl-2', status: 'pending' };

describe('STA-182 — cnextClaimHistoryToClaimRequest adapter', () => {
  it('carries the row identity + parses the amount string to a number', () => {
    const claim = cnextClaimHistoryToClaimRequest(approvedRow);
  ...
  it('maps a pending row to a pending claim with just the submit entry', () => {
    const claim = cnextClaimHistoryToClaimRequest(pendingRow);
  ...
  it('produces a PendingRequest the ClaimDetailModal can render (timeline + attachment)', () => {
    const request = benefitClaimToPendingRequest(cnextClaimHistoryToClaimRequest(approvedRow));
