// STA-182 — adapter: CnextClaimHistoryItem (the Benefits-Hub claim-history preview
...
const STATUS_MAP: Record<CnextClaimHistoryItem['status'], BenefitClaimStatus> = {
  approved: 'approved',
  pending: 'pending_spd',
...
function buildAudit(
  status: CnextClaimHistoryItem['status'],
  at: string,
  employeeName: string,
): BenefitClaimAuditEntry[] {
 ...
 */
export function cnextClaimHistoryToClaimRequest(item: CnextClaimHistoryItem): BenefitClaimRequest {
