...
export function GeofencingConfig() {
 ...
 <p className="text-sm text-ink-muted mt-1">{zone.address}</p>
 </div>
 ...
 <p className="text-xs text-ink-muted">{t('radius')}</p>
 <p className="font-medium">{zone.radius} m</p>
 </div>
 <div className="rounded-md bg-surface-raised p-2">
 <p className="text-xs text-ink-muted">{t('employees')}</p>
 <p className="font-medium">{zone.assignedEmployeeCount}</p>
 </div>
 <div className="rounded-md bg-surface-raised p-2 col-span-2">
 <p className="text-xs text-ink-muted">Coordinates</p>
 ...
 >
 <span className={cn('inline-block h-4 w-4 transform rounded-full bg-surface shadow-[var(--shadow-sm)] transition-transform duration-200', form.isActive ? 'translate-x-[18px]' : 'translate-x-0.5')} />
 </button>
 </div>
