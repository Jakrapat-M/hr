    ...
    <form onSubmit={handleSubmit}>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="rounded-lg bg-surface p-4 sm:p-6 space-y-5" style={{ boxShadow: 'var(--shadow-card)' }}>
            <FieldGroup label={t('newRequest')}>
            ...
            <FieldGroup label={t('duration')}>
              ...
              {/* Duration display */}
              {workingDays > 0 && (
                <div className="p-4 bg-accent-tint rounded-md">
                  <p className="text-xs text-ink-muted">{t('duration')}</p>
                  <p className="text-2xl font-bold text-accent">
                    {workingDays} <span className="text-sm font-normal text-ink-muted">{t('workingDays')}</span>
                  </p>
                </div>
              )}
            </FieldGroup>

            <FieldGroup label={t('reason')}>
              <FormField
                label={t('reason')}
                name="reason"
                type="textarea"
            ...
            {leaveType === 'sick' && workingDays > 3 && (
              <div className="p-4 bg-warning-tint rounded-md flex items-start gap-3">
                <AlertTriangle className="h-5 w-5 text-warning flex-shrink-0 mt-0.5" />
                <div>
                  <p className="text-sm font-medium text-warning">{t('medicalCert')}</p>
                  <p className="text-sm text-ink-soft">{t('medicalCertRequired')}</p>
                </div>
              </div>
            )}
          </div>
        ...
        <div className="space-y-4">
          {validations.length > 0 && (
            <div className="rounded-lg bg-surface p-4 space-y-3" style={{ boxShadow: 'var(--shadow-card)' }}>
              <p className="text-xs font-semibold uppercase tracking-widest text-ink-muted">Policy Check</p>
              {validations.map((v, i) => (
          ...
          {selectedBalance && (
            <div className="rounded-lg bg-surface p-4 space-y-2" style={{ boxShadow: 'var(--shadow-card)' }}>
              <p className="text-xs font-semibold uppercase tracking-widest text-ink-muted">{selectedBalance.nameEn} Balance</p>
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  <span className="text-ink-muted">{t('entitled')}</span>
                  <span className="font-medium">{selectedBalance.entitled}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-ink-muted">{t('used')}</span>
                ...
                {workingDays > 0 && (
                  <>
                    ...
                    <div className="flex justify-between">
                      <span className="text-ink-muted font-medium">After</span>
