      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        <div className="flex-1 min-w-0">
            ...
            <nav className="flex gap-0">
              {ROLES.map((role) => (
                <button
                  ...
                  className={['px-4 py-2.5 text-sm font-medium border-b-2 transition-colors', roleTab === role ? 'border-accent text-accent' : 'border-transparent text-ink-muted hover:text-ink'].join(' ')}
                >
                  {role}
                </button>
              ))}
          ...
          {filteredByRole.length === 0 && (
            <div className="py-12 text-center text-sm text-ink-muted">ไม่มี Tile สำหรับ Role นี้</div>
          )}
          ...
          <div className="hidden sm:flex flex-col gap-2">
            {filteredByRole.map((tile, index) => (
              <div
                ...
                onDrop={handleDrop}
                className={['flex items-center gap-4 rounded-lg border border-hairline bg-surface px-4 py-3 cursor-grab active:cursor-grabbing hover:bg-canvas-soft transition-colors', dragIndex === index ? 'opacity-50' : ''].join(' ')}
              >
                {/* drag handle */}
                <div className="text-ink-muted select-none" aria-hidden="true">
                ...
                <div className="flex items-center gap-2 min-w-0 flex-1">
                  <span className="text-xs bg-accent-soft text-accent px-2 py-0.5 rounded font-mono">{tile.icon}</span>
                  <div className="min-w-0">
                    <div className="text-sm font-medium text-ink truncate">{tile.label}</div>
                    <div className="flex gap-1 mt-0.5">
                      {tile.visibleTo.map((r) => (
                        <span key={r} className="text-xs bg-canvas-soft text-ink-muted px-1 rounded">{r}</span>
                      ))}
                    </div>
                  </div>
                </div>
                ...
                <div className="flex gap-1 shrink-0">
                  {SIZES.map((s) => (
                    <button
                      ...
                      className={['w-8 h-8 text-xs rounded border font-medium transition-colors', tile.size === s ? 'bg-accent text-white border-accent' : 'bg-surface text-ink-muted border-hairline hover:border-accent'].join(' ')}
                    >
                      {s}
                    </button>
                  ))}
                ...
                <button
                  ...
                  className={['relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-accent', tile.enabled ? 'bg-accent' : 'bg-surface-raised'].join(' ')}
                >
                  <span className={['pointer-events-none inline-block h-4 w-4 rounded-full bg-surface shadow-card ring-0 transition-transform duration-200', tile.enabled ? 'translate-x-4' : 'translate-x-0'].join(' ')} />
                </button>

                {/* Edit */}
                <button type="button" onClick={() => openEdit(tile)} className="text-sm text-accent hover:underline shrink-0">แก้ไข</button>
              </div>
            ))}
          </div>
          ...
          <div className="sm:hidden space-y-3">
            {filteredByRole.map((tile) => (
              <div key={tile.id} className="rounded-lg border border-hairline bg-surface p-4">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <span className="text-xs bg-accent-soft text-accent px-2 py-0.5 rounded font-mono mr-1">{tile.icon}</span>
                    <span className="text-sm font-medium text-ink">{tile.label}</span>
                  </div>
                  <div className="flex flex-col gap-1 shrink-0">
                    <button type="button" onClick={() => moveUp(tile.id)} aria-label="เลื่อนขึ้น" className="w-8 h-8 flex items-center justify-center rounded border border-hairline text-ink-muted hover:bg-canvas-soft">▲</button>
                    <button type="button" onClick={() => moveDown(tile.id)} aria-label="เลื่อนลง" className="w-8 h-8 flex items-center justify-center rounded border border-hairline text-ink-muted hover:bg-canvas-soft">▼</button>
                  </div>
                </div>
                {/* Size + edit row */}
                <div className="mt-3 flex items-center gap-3">
                  {SIZES.map((s) => (
                    <button key={s} type="button" onClick={() => changeSize(tile.id, s)} aria-pressed={tile.size === s}
                      className={['w-8 h-8 text-xs rounded border font-medium', tile.size === s ? 'bg-accent text-white border-accent' : 'bg-surface text-ink-muted border-hairline'].join(' ')}>
                      {s}
                    </button>
                  ))}
                  <button type="button" onClick={() => openEdit(tile)} className="ml-auto text-sm text-accent hover:underline">แก้ไข</button>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="lg:w-80 shrink-0">
          <div className="bg-surface rounded-lg border border-hairline p-4 sticky top-4">
            <h3 className="text-sm font-medium text-ink mb-3">
              Preview: <span className="text-accent">{roleTab}</span>
            </h3>
            {/* 4-col CSS grid */}
            <div className="grid grid-cols-4 gap-2 auto-rows-[56px]">
              {filteredByRole.filter((t) => t.enabled).map((tile) => (
                <div
                  key={tile.id}
                  className={[
                    'rounded-md bg-accent-soft border border-hairline flex flex-col items-center justify-center px-1 min-w-0',
                    SIZE_SPAN[tile.size],
                  ].join(' ')}
              ...
              {filteredByRole.filter((t) => t.enabled).length === 0 && (
                <div className="col-span-4 py-6 text-center text-xs text-ink-muted">ไม่มี Tile ที่เปิดใช้สำหรับ Role นี้</div>
              )}
            </div>
          </div>
        </div>
      ...
      {editTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" role="dialog" aria-modal="true" aria-label={`แก้ไข Tile ${editTarget.label}`}>
          <div className="bg-surface rounded-lg shadow-card w-full max-w-md p-6">
            <h2 className="text-base font-semibold text-ink mb-4">แก้ไข Tile</h2>

            <div className="mb-4">
              <label className="block text-sm font-medium text-ink mb-1.5">ชื่อ</label>
              ...
              <div className="flex gap-3">
                {SIZES.map((s) => (
                  <button key={s} type="button" onClick={() => setModalSize(s)} aria-pressed={modalSize === s}
                    className={['flex-1 py-2 text-sm rounded-md border font-medium', modalSize === s ? 'bg-accent text-white border-accent' : 'bg-surface text-ink-muted border-hairline hover:border-accent'].join(' ')}>
                    {SIZE_LABEL[s]}
                  </button>
                ))}
              </div>
            ...
            <div className="mb-6">
              <label className="block text-sm font-medium text-ink mb-2">Roles ที่มองเห็น</label>
                ...
                {ROLES.map((role) => {
                  ...
                  return (
                    <button
                      ...
                      className={['px-3 py-2 text-sm rounded-md border transition-colors', selected ? 'bg-accent text-white border-accent' : 'bg-surface text-ink-muted border-hairline hover:border-accent'].join(' ')}
                    >
                      {role}
                    </button>
                  )
