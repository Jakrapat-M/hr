...
export default function QuickActionsPage() {
  ...
  return (
    ...
    <EditorShell editor="quick-actions" titleTh="จัดการ Quick Actions" brd="#182">
      ...
      <div className="hidden sm:block rounded-lg border border-hairline overflow-hidden">
        <div className="bg-canvas-soft border-b border-hairline grid grid-cols-[32px_1fr_80px_80px_96px] gap-4 px-4 py-3 text-xs font-medium text-ink-muted uppercase">
          <div />
          <div>ชื่อ / Icon</div>
        ...
        {quickActions.map((qa, index) => (
          <div
            ...
            onDrop={handleDrop}
            className={[
              'grid grid-cols-[32px_1fr_80px_80px_96px] gap-4 px-4 py-3.5 items-center border-b border-hairline bg-surface',
              'hover:bg-canvas-soft cursor-grab active:cursor-grabbing',
              dragIndex === index ? 'opacity-50' : '',
            ].join(' ')}
          >
            {/* drag handle */}
            <div className="text-ink-muted select-none text-center" aria-hidden="true">
            ...
            <div className="flex items-center gap-3 min-w-0">
              <span className="text-xs bg-canvas-soft text-ink-muted px-2 py-1 rounded font-mono">{qa.icon}</span>
              <div className="min-w-0">
                <div className="text-sm font-medium text-ink truncate">{qa.label}</div>
                <div className="text-xs text-ink-muted truncate">{qa.href}</div>
              </div>
            </div>
            ...
            <div className="flex justify-center">
              <button
                ...
                className={['relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-accent focus:ring-offset-1', qa.enabled ? 'bg-accent' : 'bg-surface-raised'].join(' ')}
              >
                <span className={['pointer-events-none inline-block h-4 w-4 rounded-full bg-surface shadow-card ring-0 transition-transform duration-200', qa.enabled ? 'translate-x-4' : 'translate-x-0'].join(' ')} />
              </button>
            </div>

            {/* Order badge */}
            <div className="text-sm text-ink-muted font-mono text-center">{qa.order}</div>
      ...
      <div className="sm:hidden space-y-3">
        {quickActions.map((qa, index) => (
          <div key={qa.id} className="rounded-lg border border-hairline bg-surface p-4">
            <div className="flex items-start justify-between gap-3">
              <div>
                <span className="text-xs bg-canvas-soft text-ink-muted px-2 py-0.5 rounded font-mono mr-2">{qa.icon}</span>
                <span className="text-sm font-medium text-ink">{qa.label}</span>
                <div className="text-xs text-ink-muted mt-1">{qa.href}</div>
              </div>
              ...
              <div className="flex flex-col gap-1 shrink-0">
                <button
                  ...
                  className="w-8 h-8 flex items-center justify-center rounded border border-hairline text-ink-muted hover:bg-canvas-soft disabled:opacity-30 disabled:cursor-not-allowed"
                >
                  ▲
                </button>
                <button
            ...
            <div className="mt-3 flex items-center gap-3">
              <button
                ...
                aria-label={`เปิดใช้ ${qa.label}`}
                onClick={() => toggleEnabled(qa.id)}
                className={['relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-accent', qa.enabled ? 'bg-accent' : 'bg-surface-raised'].join(' ')}
              >
                <span className={['pointer-events-none inline-block h-4 w-4 rounded-full bg-surface shadow-card ring-0 transition-transform duration-200', qa.enabled ? 'translate-x-4' : 'translate-x-0'].join(' ')} />
              </button>
              <span className="text-sm text-ink-muted">{qa.enabled ? 'เปิดใช้งาน' : 'ปิดใช้งาน'}</span>
              <button type="button" onClick={() => openEdit(qa)} className="ml-auto text-sm text-accent hover:underline">
                แก้ไข
              </button>
            </div>
          </div>
      ...
      {editTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" role="dialog" aria-modal="true" aria-label={`แก้ไข ${editTarget.label}`}>
          <div className="bg-surface rounded-lg shadow-card w-full max-w-md p-6">
            <h2 className="text-base font-semibold text-ink mb-4">แก้ไข Quick Action</h2>
            ...
            <div className="mb-6">
              <label className="block text-sm font-medium text-ink mb-1.5">Route (href)</label>
