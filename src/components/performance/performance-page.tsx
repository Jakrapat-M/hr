...
export function PerformancePage() {
 ...
 <p className="text-ink-muted mt-1">{t('weightCurrentTotal')}: {totalWeight}% / 100%</p>
 </div>
 ...
 <p className="text-sm text-ink-muted mb-3">{goal.descriptionEn}</p>
 <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
 <div><span className="text-ink-muted">{t('weight')}</span><p className="font-medium">{goal.weight}%</p></div>
 <div><span className="text-ink-muted">{t('target')}</span><p className="font-medium">{goal.targetValue}</p></div>
 <div><span className="text-ink-muted">{t('actual')}</span><p className="font-medium">{goal.actualValue}</p></div>
 <div><span className="text-ink-muted">{t('progress')}</span><p className="font-medium">{goal.targetValue > 0 ? Math.round((goal.actualValue / goal.targetValue) * 100) : 0}%</p></div>
 </div>
 <div className="mt-3 w-full bg-surface-raised rounded-full h-2">
 ...
 <p className="text-xs text-ink-muted">{tEval('rating.finalRating')}</p>
 ...
 <p className="text-sm text-ink-muted mb-2">{review.summary}</p>
