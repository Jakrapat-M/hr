...
function ApprovalTimeline({ steps }: { steps: ApprovalStep[] }) {
 ...
 <ol className="relative border-l border-hairline border-hairline ml-3">
 {steps.map((step) => (
...
'absolute -left-2 w-4 h-4 rounded-full border-2 border-white',
 ...
 {step.comment && <p className="text-xs text-ink-muted mt-1">{step.comment}</p>}
 </li>
 ))}
 </ol>
 </div>
...
'fixed inset-y-0 right-0 z-50 w-full sm:w-[480px] bg-surface shadow-[var(--shadow-lg)] transform transition-transform duration-300 flex flex-col',
 ...
 <span className="text-ink-muted">{typeIcons[request.type] ?? <ClipboardList className="h-5 w-5" />}</span>
 <div>
 ...
 <button
 onClick={onClose}
 className="rounded-md p-1.5 hover:bg-surface-raised hover:bg-surface-raised transition-colors"
