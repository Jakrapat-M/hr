...
import { Card, CardTitle, Button, DemoValuesDisclaimer } from '@/components/cnext';
...
export function GovernmentReports() {
 ...
 return <div className="text-center py-12"><p className="text-ink-muted">{t('common.noData')}</p></div>;
 }
 ...
 <th className="text-left py-2 px-3 font-medium text-ink-muted">{t('govReports.type')}</th>
 <th className="text-left py-2 px-3 font-medium text-ink-muted">{t('govReports.period')}</th>
 <th className="text-left py-2 px-3 font-medium text-ink-muted">{t('govReports.generatedDate')}</th>
 <th className="text-left py-2 px-3 font-medium text-ink-muted">{t('govReports.records')}</th>
 <th className="text-left py-2 px-3 font-medium text-ink-muted">{t('govReports.status')}</th>
 <th className="text-right py-2 px-3 font-medium text-ink-muted">{t('common.actions')}</th>
 </tr>
 </thead>
 <tbody>
 {reportHistory.map((report) => (
 <tr key={report.id} className="border-b border-hairline hover:bg-surface-raised/30">
