...
function Stepper({ currentStage, stages, t }: { currentStage: PayrollStage; stages: PayrollStage[]; t: (key: string) => string }) {
 ...
 <span className={`text-xs mt-1 ${isCurrent ?'text-brand font-medium' :'text-ink-muted'}`}>
...
export function PayrollProcessing() {
 ...
 <th
 key={col.key}
 className="text-left py-2 px-3 font-medium text-ink-muted cursor-pointer hover:text-ink"
 ...
 <tr key={ps.employeeId} className="border-b border-hairline hover:bg-surface-raised/30">
 ...
 <>
 <p className="text-ink-muted mb-2">{t('payrollProcessing.period')}: {runSummary.period}</p>
 <p className="text-ink-muted mb-2">{t('payrollProcessing.totalEmployees')}: {runSummary.totalEmployees}</p>
