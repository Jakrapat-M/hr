...
export function MedicalClaimsPage() {
 ...
 isActive ?'text-brand font-medium' :'text-ink-muted'
 }`}
 >
 {s.label}
 </span>
 ...
 step > s.num ?'bg-brand' :'bg-surface-raised '
 }`}
 />
 )}
 </div>
 ...
 <div className="space-y-2">
 {otherFiles.map((f, i) => (
 <div key={i} className="flex items-center justify-between rounded-md border border-hairline border-hairline bg-surface-raised p-3">
 <div className="flex items-center gap-2 min-w-0">
 <FileText className="h-4 w-4 text-ink-muted shrink-0" />
 <span className="text-sm text-ink-soft truncate">{f.name}</span>
 <span className="text-xs text-ink-muted shrink-0">{formatFileSize(f.size)}</span>
 </div>
 <button
 ...
 <span className="text-sm text-ink-muted">{usagePercent}%</span>
 </div>
 ...
 <div
 key={claim.id}
 className="flex items-center justify-between rounded-md border border-hairline p-3 hover:bg-surface-raised/50 cursor-pointer transition-colors"
 ...
 <div>
 <dt className="text-xs text-ink-muted">{t('medicalClaims.form.hospitalName')}</dt>
 <dd className="text-sm font-medium">{form.hospitalName}</dd>
 </div>
 <div>
 <dt className="text-xs text-ink-muted">{t('medicalClaims.form.treatmentDate')}</dt>
 <dd className="text-sm font-medium">{formatDate(form.treatmentDate)}</dd>
 </div>
 <div>
 <dt className="text-xs text-ink-muted">{t('medicalClaims.form.receiptAmount')}</dt>
 <dd className="text-sm font-medium">{formatCurrency(parseFloat(form.receiptAmount) || 0)}</dd>
 </div>
 <div>
 <dt className="text-xs text-ink-muted">{t('medicalClaims.form.claimAmount')}</dt>
 <dd className="text-sm font-medium">{formatCurrency(parseFloat(form.claimAmount) || 0)}</dd>
 </div>
 {form.remarks && (
 <div className="sm:col-span-2">
 <dt className="text-xs text-ink-muted">{t('medicalClaims.form.remarks')}</dt>
 ...
 <span className="text-xs text-ink-muted">{formatFileSize(f.size)}</span>
 ...
 <thead>
 <tr className="border-b border-hairline bg-surface-raised text-left">
 <th className="px-4 py-3 font-medium text-ink-muted">{t('medicalClaims.history.id')}</th>
 <th className="px-4 py-3 font-medium text-ink-muted">{t('medicalClaims.history.disease')}</th>
 <th className="px-4 py-3 font-medium text-ink-muted">{t('medicalClaims.history.hospital')}</th>
 <th className="px-4 py-3 font-medium text-ink-muted">{t('medicalClaims.history.date')}</th>
 <th className="px-4 py-3 font-medium text-ink-muted">{t('medicalClaims.history.amount')}</th>
 <th className="px-4 py-3 font-medium text-ink-muted">{t('medicalClaims.history.status')}</th>
 <th className="px-4 py-3 font-medium text-ink-muted">{t('medicalClaims.history.action')}</th>
 </tr>
 </thead>
 <tbody>
 {filteredClaims.map((claim) => (
 <tr key={claim.id} className="border-b border-hairline hover:bg-surface-raised/30 transition-colors">
 ...
 <p className="text-xs text-ink-muted mt-0.5">{formatDate(claim.treatmentDate)}</p>
 </div>
 <Badge variant={STATUS_VARIANT[claim.status]}>
 {t(`medicalClaims.status.${claim.status}`)}
 </Badge>
 ...
 <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3">
 <div>
 <dt className="text-xs text-ink-muted">{t('medicalClaims.detail.disease')}</dt>
 <dd className="text-sm font-medium">{getCategoryLabel(detailClaim.diseaseCategory)}</dd>
 </div>
 <div>
 <dt className="text-xs text-ink-muted">{t('medicalClaims.detail.details')}</dt>
 <dd className="text-sm font-medium">{detailClaim.diseaseDetails ||'-'}</dd>
 </div>
 <div>
 <dt className="text-xs text-ink-muted">{t('medicalClaims.detail.hospital')}</dt>
 <dd className="text-sm font-medium">{detailClaim.hospitalName}</dd>
 </div>
 <div>
 <dt className="text-xs text-ink-muted">{t('medicalClaims.detail.treatmentDate')}</dt>
 <dd className="text-sm font-medium">{formatDate(detailClaim.treatmentDate)}</dd>
 </div>
 <div>
 <dt className="text-xs text-ink-muted">{t('medicalClaims.detail.receiptAmount')}</dt>
 <dd className="text-sm font-medium">{formatCurrency(detailClaim.receiptAmount)}</dd>
 </div>
 <div>
 <dt className="text-xs text-ink-muted">{t('medicalClaims.detail.claimAmount')}</dt>
 <dd className="text-sm font-medium">{formatCurrency(detailClaim.claimAmount)}</dd>
 </div>
 {detailClaim.remarks && (
 <div className="sm:col-span-2">
 <dt className="text-xs text-ink-muted">{t('medicalClaims.detail.remarks')}</dt>
 ...
 <li key={doc.id} className="flex items-center gap-2 rounded-md border border-hairline p-2">
 <FileText className="h-4 w-4 text-ink-muted shrink-0" />
 <span className="text-sm truncate flex-1">{doc.fileName}</span>
 <span className="text-xs text-ink-muted">{formatFileSize(doc.fileSize)}</span>
 </li>
 ))}
 </ul>
 </div>
 ...
 {/* Vertical line */}
 {!isLast && (
 <div className="absolute left-[9px] top-5 h-full w-0.5 bg-surface-raised" />
 )}
 {/* Dot */}
 <div
 ...
 <p className="text-xs text-ink-muted">{as.approverName}</p>
 {as.date && (
 <p className="text-xs text-ink-muted mt-0.5">{formatDate(as.date)}</p>
 )}
 {as.comment && (
 <p className="text-xs text-ink-muted mt-1 italic">{as.comment}</p>
 )}
 </div>
 </div>
 );
