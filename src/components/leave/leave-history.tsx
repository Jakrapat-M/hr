...
export function LeaveHistory({
  ...
  const getActionCell = (request: LeaveRequest, isMobile = false) => {
    if (request.status === 'pending' && onCancelPending) {
      return (
        <Button
          ...
          className={cn('text-danger border-danger/30 hover:bg-danger-tint', isMobile && 'text-xs')}
        >
          {t('cancelRequest')}
        </Button>
      );
    ...
    if (canRequestCancellation) {
      return (
        <Button
          ...
          className={cn('text-warning border-warning/30 hover:bg-warning-tint', isMobile && 'text-xs')}
        >
    ...
    if (request.cancellationRequested) {
      return <span className="text-xs text-warning">{t('cancellationPendingApproval')}</span>;
    }
          ...
          <>
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-hairline">
                  {paginatedItems.map((req) => {
                    ...
                    return (
                      <tr key={req.id} className="hover:bg-surface-raised/40 transition cursor-pointer" onClick={() => onViewDetail(req)}>
            ...
            <div className="md:hidden divide-y divide-hairline">
              {paginatedItems.map((req) => {
                const colors = LEAVE_TYPE_COLORS[req.type as LeaveType] || LEAVE_TYPE_COLORS.annual;
                return (
                  <div key={req.id} className="p-4 hover:bg-surface-raised/40 transition cursor-pointer" onClick={() => onViewDetail(req)}>
