...
export function IdpPage() {
 ...
 <div className="w-40 text-sm font-medium text-ink-soft truncate">{gap.name}</div>
 <div className="flex-1">
 <div className="flex items-center gap-2 mb-1">
 <span className="text-xs text-ink-muted">{t('currentLevel')}: {gap.currentLevel}</span>
 <span className="text-xs text-ink-muted">{t('requiredLevel')}: {gap.requiredLevel}</span>
 ...
 <div className="mt-2 w-full bg-surface-raised rounded-full h-1.5">
 <div className="bg-brand h-1.5 rounded-full transition-all" style={{ width: `${action.progress}%` }} />
 </div>
 </div>
 ...
 <span className="text-sm text-ink-muted">{avgProgress}% ({actions.filter((a) => a.status ==='completed').length}/{actions.length} {t('completed')})</span>
 </div>
 <div className="w-full bg-surface-raised rounded-full h-2.5">
 <div className="bg-brand h-2.5 rounded-full transition-all" style={{ width: `${avgProgress}%` }} />
 </div>
 </div>
 );
 })}
