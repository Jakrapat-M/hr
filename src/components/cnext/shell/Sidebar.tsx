...
// ════════════════════════════════════════════════════════════
// Cnext Sidebar — ported 1:1 from the HRMS Blueprint shell
...
//    and does not collide with the legacy `.cnext-*` sidebar rules.
// ════════════════════════════════════════════════════════════
...
export interface SidebarProps {
  ...
  onClose?: () => void;
  /** Extra className merged onto <aside> — e.g. "cnext-sidebar--drawer". */
  className?: string;
}
...
export const MODULES: ModuleGroup[] = [
  ...
  },
  {
    ...
    leaves: [
      ...
      { id: 'team-overview', label: 'Time Management', labelTh: 'จัดการเวลา', href: '/team/time-management', show: ['manager', 'hradmin', 'hris'] }, // STA-245 → STA-255 — manager time surface (approvals-first + compact KPI dashboard); same guard as roster, remove-not-hide for others
      { id: 'roster', label: 'Roster & Shifts', labelTh: 'ตารางกะ', href: '/roster', show: ['manager', 'hradmin', 'hris'] }, // repointed → real /roster page
      { id: 'shift-assign', label: 'Shift Assignment', labelTh: 'จัดกะให้พนักงาน', href: '/team/shift-assign', badge: 'ใหม่', show: ['manager', 'hradmin', 'hris'] }, // STA-168 — manager month-grid scheduler; remove-not-hide for other roles (HRBP/HR reach the review grid via the /quick-approve deep-link)
...
export function Sidebar({ onNavigate, onClose, className }: SidebarProps = {}) {
  ...
  const isDrawer = (className ?? '').includes('cnext-sidebar--drawer');
  ...
  // --cnext-sidebar-w, which the .cnext-app grid in globals.css consumes. Clamped
  ...
  useEffect(() => {
    if (isDrawer) return;
    try {
      document.documentElement.style.setProperty('--cnext-sidebar-w', `${width}px`);
    } catch {
      /* non-DOM env — ignore. */
    }
  }, [width, isDrawer]);
  ...
  return (
    <aside
      className={cn('cnext-sidebar bp-shellnav', collapsedDesktop && 'bp-collapsed', className)}
      aria-label="เมนูหลัก"
    >
      ...
      <div className="bp-rail">
        ...
        {onClose && (
          <button
            type="button"
            className="cnext-icon-btn bp-rail-close"
            aria-label="ปิดเมนู"
            onClick={onClose}
          >
