...
beforeEach(() => {
  ...
  authMock.email = 'employee@cnext.test';
...
describe('PersonaSwitcher — SF realignment', () => {
  ...
  it('AC4: search input filters the persona list', () => {
    ...
    const apinya = DEMO_USERS['apinya@cnext.test'];
    expect(screen.getByText(apinya.name)).toBeInTheDocument();
    // Another persona should be gone
    expect(screen.queryByText(DEMO_USERS['ken@cnext.test'].name)).toBeNull();
  });

  it('AC4: search filter is case-insensitive and matches email too', () => {
    render(<PersonaSwitcher />);
    const search = screen.getByPlaceholderText('ค้นหาด้วยชื่อ อีเมล หรือบทบาท');
    fireEvent.change(search, { target: { value: 'WORAWEE@CNEXT' } });
    expect(screen.getByText(DEMO_USERS['worawee@cnext.test'].name)).toBeInTheDocument();
    expect(screen.queryByText(DEMO_USERS['ken@cnext.test'].name)).toBeNull();
  });
  ...
  it('AC4: search filter matches by role label (HRBP)', () => {
    ...
    expect(screen.getByText(DEMO_USERS['hrbp@cnext.test'].name)).toBeInTheDocument();
    expect(screen.getByText(DEMO_USERS['apinya@cnext.test'].name)).toBeInTheDocument();
  });

  it('AC5: each row renders a secondary line with jobTitle · department', () => {
    render(<PersonaSwitcher />);
    const ken = DEMO_USERS['ken@cnext.test'];
  ...
  it('Tier chips A·B·C·D still render for the super-user row', () => {
    render(<PersonaSwitcher />);
    const adminRow = screen.getByText(DEMO_USERS['admin@cnext.test'].name).closest('button')!;
    ['A', 'B', 'C', 'D'].forEach((t) =>
  ...
  it('Selecting a non-active row calls switchPersona + locale-prefixed landing', () => {
    render(<PersonaSwitcher />);
    fireEvent.click(screen.getByText(DEMO_USERS['admin@cnext.test'].name).closest('button')!);
    expect(switchPersonaMock).toHaveBeenCalledTimes(1);
    const landing = landingForDemoUser('admin@cnext.test', 'th');
  ...
  it('Active row is disabled and labeled "ใช้อยู่"', () => {
    ...
    const activeRow = screen
      .getByText(DEMO_USERS['employee@cnext.test'].name)
      .closest('button')!;
...
describe('PersonaSwitcher — Topbar wiring (AC1/AC2)', () => {
  ...
  it('AC1: Topbar.tsx no longer renders a standalone <PersonaSwitcher /> pill trigger', async () => {
    ...
    const topbar = fs.readFileSync(
      path.resolve(process.cwd(), 'src/components/cnext/shell/Topbar.tsx'),
      'utf8',
    );
