...
// Legacy burnt-orange hex kept out of source as a literal (Cnext design-check
...
describe('LoginAsRibbon — shell wiring', () => {
  const shellDir = path.resolve(process.cwd(), 'src/components/cnext/shell');
  const appShell = fs.readFileSync(path.join(shellDir, 'AppShell.tsx'), 'utf8');
  const barrel = fs.readFileSync(
    path.resolve(process.cwd(), 'src/components/cnext/index.ts'),
    'utf8',
  );
  ...
  it('AC2.6: the ActingBadge barrel export is gone from @/components/cnext', () => {
