...
import { CNEXT_DEPENDENTS, type CnextDependent } from '@/lib/cnext-mock-data';
...
export const useCnextProfileStore = create<ProfileState>()(
  persist(
    (set, get) => ({
      activeTab: 'personal',
      isEditing: false,
      ...
      adminApproveWithReason: (changeId, reason) => {
        ...
        if (!change) {
          console.warn('[cnext-profile-slice] adminApproveWithReason: change not found', changeId);
          return;
        }
        set((s) => ({
          pendingChanges: s.pendingChanges.map((pc) =>
      ...
      adminRejectWithReason: (changeId, reason) => {
        ...
        const change = pendingChanges.find((pc) => pc.id === changeId);
        if (!change) {
          console.warn('[cnext-profile-slice] adminRejectWithReason: change not found', changeId);
          return;
        }
        set((s) => ({
          pendingChanges: s.pendingChanges.map((pc) =>
      ...
      withdrawPendingChange: (changeId) => {
        ...
        const change = pendingChanges.find((pc) => pc.id === changeId);
        if (!change) {
          console.warn('[cnext-profile-slice] withdrawPendingChange: change not found', changeId);
          return;
        }
        if (change.status !== 'pending') {
          console.warn('[cnext-profile-slice] withdrawPendingChange: change is not pending', changeId);
          return;
        }
        set((s) => ({
          pendingChanges: s.pendingChanges.map((pc) =>
    ...
    {
      name: 'cnext-profile-v1',          // KEEP name — version controls migration
      version: 6,
