...
type ReferralStatus = 'pending_hrbp' | 'spd_reviewing' | 'approved' | 'letter_issued' | 'rejected' | 'send_back';
...
const STATUS_LABEL_TH: Record<ReferralStatus, string> = {
  pending_hrbp: 'รอ HRBP อนุมัติ',
...
const STATUS_LABEL_EN: Record<ReferralStatus, string> = {
...
function ReferralRow({ referral, locale }: { referral: MockReferral; locale: string }) {
  const [expanded, setExpanded] = useState(false);
  const isPending = referral.status === 'pending_hrbp' || referral.status === 'spd_reviewing';
  ...
  const activeStage: ApproverStage | undefined =
    referral.status === 'pending_hrbp' ? 'hrbp' :
    referral.status === 'spd_reviewing' ? 'spd' :
    undefined;
      ...
      <div className="flex flex-col gap-3">
        <div className="flex items-start justify-between gap-3 flex-wrap">
          ...
          <div className="flex flex-col items-end gap-1 shrink-0">
            <span className={cn('rounded-full px-2.5 py-0.5 text-xs font-medium whitespace-nowrap', STATUS_STYLE[referral.status])}>
              {statusLabel}
            </span>
            {isPending && (
...
export default function HospitalReferralPage() {
  ...
  const pendingCount = MOCK_REFERRALS.filter((r) => r.status === 'pending_hrbp' || r.status === 'spd_reviewing').length;
  const letterCount = MOCK_REFERRALS.filter((r) => r.status === 'letter_issued').length;

  return (
    <div className="pb-8 flex flex-col gap-6">
      <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div className="flex flex-col gap-1">
          <CardEyebrow>{locale === 'th' ? 'สวัสดิการ · Hospital referral' : 'Benefits · Hospital referral'}</CardEyebrow>
          <h1 className="font-display text-[length:var(--text-display-h1)] font-semibold leading-[var(--text-display-h1--line-height)] tracking-tight text-ink">
            {locale === 'th' ? 'ขอใบส่งตัว / ePatient' : 'Hospital Referral Requests'}
          </h1>
          <p className="max-w-2xl text-body leading-relaxed text-ink-soft">
            {locale === 'th'
              ...
              : 'This route is for SPD to issue a referral letter before your visit — not for post-visit reimbursement.'}
          </p>
        </div>
        <div className="flex flex-wrap gap-2 shrink-0">
          <Link href={benefitsHubRoute(locale)} className={buttonVariants({ variant: 'ghost' })}>
            <ArrowLeft size={14} aria-hidden />
            {locale === 'th' ? 'กลับ Benefits Hub' : 'Back to Benefits Hub'}
          </Link>
          <Link href={benefitReferralRoute(locale)} className={buttonVariants({ variant: 'primary' })}>
            {locale === 'th' ? 'ยื่นคำขอใหม่' : 'New Referral Request'}
          </Link>
        </div>
      </header>
      ...
      <div className="flex gap-3 flex-wrap">
        ...
        {letterCount > 0 && (
          <span className="rounded-full px-3 py-1 text-xs font-medium bg-green-50 text-green-700 border border-green-200">
            {locale === 'th' ? 'ออกใบส่งตัวแล้ว' : 'Letter Issued'} · {letterCount}
          </span>
        )}
      </div>
      ...
      <section>
        <h2 className="font-display text-[length:var(--text-display-h3)] font-semibold text-ink mb-4">
          {locale === 'th' ? 'ประวัติคำขอใบส่งตัว' : 'Referral History'}
        </h2>
        <ul className="flex flex-col gap-3" aria-label={locale === 'th' ? 'รายการคำขอใบส่งตัว' : 'Referral requests'}>
          {MOCK_REFERRALS.map((r) => (
            <ReferralRow key={r.id} referral={r} locale={locale} />
          ))}
        </ul>
      </section>
    </div>
